...
class DAGEstimator(GenerateData):
    ...
    def PEM(self, ite):
        ...
        if self.distribution == "Poisson":
            lb = [np.zeros(self.K + 1) for _ in range(self.J)]
            ub = [np.concatenate(([2], 3 * np.ones(self.K))) for _ in range(self.J)]
        ...
        if self.distribution == "Lognormal":
            lb = [np.concatenate(([-2], np.zeros(self.K + 1))) for _ in range(self.J)]
            ub = [np.concatenate((4 * np.ones(self.K + 1), [2])) for _ in range(self.J)]
            while abs(err) > self.tol and itera < self.max_iter:
                old_loglik = loglik
                phi = np.zeros((self.N, n_in))
                psi = np.zeros(n_in).reshape(n_in, 1)
        ...
        raise NotImplementedError("PEM is only implemented here for Poisson and Lognormal, matching your original file.")
